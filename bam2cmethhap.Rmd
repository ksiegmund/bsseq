...
Now run pipeline for a single sample.  Sorted Bam file + index get saved to a new directory. (need to create the directory)
```{r sort-index-bam}
start_time <- Sys.time()

sortBam(file.path(datadir,bamfiles)[whichbam],
        file.path("data/sortedbams",
                  paste(snm[whichbam],"sorted",sep=".")))
indexBam(file.path("data/sortedbams",
                   paste(snm[whichbam],"sorted.bam",sep=".")))
```
 
```{r read-bam} 
gal <- readGAlignments(file.path("data/sortedbams",
              paste(snm[whichbam],"sorted.bam",sep=".")), use.names=TRUE, param=param1)
